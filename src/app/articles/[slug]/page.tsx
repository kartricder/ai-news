...
export default async function ArticleDetailPage(props: PageProps<'/articles/[slug]'>) {
  ...
  const content = cleanContent(article.contentVi || article.summaryVi);
  ...
  return (
    <article className="mx-auto max-w-3xl py-8">
      ...
      <header className="mt-6 border-b border-slate-200 pb-6">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-slate-950 md:text-5xl">{article.title}</h1>
        <p className="mt-4 text-lg leading-8 text-slate-600">{article.summaryVi}</p>
